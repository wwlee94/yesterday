...
//로그인 서버와 연결하는 클래스
public class DeleteGoalServer extends AsyncTask<Void,Void,String> {
    ...
    public DeleteGoalServer(String userID,String food) {
        this.userID = userID;
        this.food = food;
    }
    ...
    protected String doInBackground(Void... params) {
        ...
        requestBody = new FormBody.Builder().add("USERID",userID).add("FOOD",food).build();

        // post형식으로 url로 만든 body를 보냄
        Request request = new Request.Builder()
                .url("http://"+ WEBIP + ":80/skuniv/deleteGoal")
                .post(requestBody)
                .build();
        try {
